...
<section class="shop_section layout_padding">
  <div class="container">
    <div class="heading_container heading_center mt-5">
      <h2>
        Sản phẩm được tìm kiếm với kết quả:
      </h2>
    </div>
    <div class="row">
      <?php
      if (isset($search_product) && mysqli_num_rows($search_product) > 0) {
        while ($row = mysqli_fetch_array($search_product)) { ?>
          <div class="col-sm-6 col-md-4 col-lg-3">
            <div class="box">
              <a href="index.php?act=detail&id=<?php echo $row['MA_XE']; ?>">
                <div class="img-box">
                  <img src="./admin/images/uploads/<?php echo $row["HINH_ANH"]; ?>" alt="<?php echo $row["TEN_XE"]; ?>">
                </div>
              ...
              <form method="POST" action="index.php?act=search" class="d-flex justify-content-center">
                <input type="hidden" name="ma_xe" value="<?php echo $row['MA_XE']; ?>">
                <button type="submit" name="add_to_cart" class="btn btn-primary w-100 mt-3">Thêm vào giỏ hàng</button>
              </form>
            </div>
          </div>
        <?php }
      } else {
        echo "<p class='col-12 text-center'>Không tìm thấy sản phẩm!</p>";
      }
      ?>
    </div>
